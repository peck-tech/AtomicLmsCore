    ...
    where TRequest : notnull
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        telemetryClient.TrackEvent($"{requestName}.Started",
        ...
        try
        {
            ...
            telemetryClient.TrackEvent($"{requestName}.Completed",
                new Dictionary<string, string>
                {
        ...
        catch (Exception ex)
        {
            ...
            telemetryClient.TrackEvent($"{requestName}.Failed",
                new Dictionary<string, string>
                {
                    { "Duration", stopwatch.ElapsedMilliseconds.ToString() }, { "ExceptionType", ex.GetType().Name }
                });

            logger.LogError(ex, "Error processing {RequestName} after {ElapsedMilliseconds}ms", requestName,
                stopwatch.ElapsedMilliseconds);
